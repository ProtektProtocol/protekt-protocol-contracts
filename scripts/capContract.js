...
if (!argv.network) {
  console.log(
      'Usage: node scripts/capContract --network kovan' 
  );
  process.exit();
}
...
const capContract = async (contractAddress, contractABI, args, from, privateKey, web3, network) => {
  try{
      ...
      let tx = await createTx(protektContract, 'capDeposits' ,from,contractAddress,args,web3)
      console.log('got tx...')
      console.log(tx)
...
(async function(){
  ...
  const contractAddress = "0xE16259D3174698c4C21A7188E4866e1A8Ced480C"
  ...
  let capAmountWei = BigInt(23 * 1e6)
  let args = [capAmountWei.toLocaleString('fullwide',{useGrouping:false})]

  console.log(`capping contract ${contractAddress} at (${capAmountWei})`)
  console.log(args)

  try{
    let result = capContract(contractAddress,ABI,args,PUBLIC_KEY,PRIVATE_KEY,web3,1)
    console.log(result)
  }catch(e){
    console.log(e)
  }
